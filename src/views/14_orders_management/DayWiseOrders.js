...
export default function DayWiseOrders(props) {
    ...
    const callback = (res) => {
        const today = new Date();console.log(res.data);

            setData(res.data.filter(a1 => new Date(a1.order_date).setHours(0, 0, 0, 0) === today.setHours(0, 0, 0, 0)));
    ...
    const showOrders = () => {
        ...
        Axios.get(apis.base+apis.adminAllOrders, {
            headers: {
                userId: 30
            }
        }).then((succResp) => {
