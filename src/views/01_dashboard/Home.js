...
export default function Home(props) {
    ...
    React.useEffect(() => {
        ...
        const fetchCancelledOrdersData = () => {
            Axios.get(apis.base + apis. ThisWeekCancelledOrderPercentage, {
                headers: {
                    userId: 30
                }
            }).then((succResp) => {
                callbackCancelledOrders(succResp); 
            }).catch((errorresp) => {
                console.log(JSON.stringify(errorresp));
            });
        };
        const fetchPendingOrdersData = () => {
            Axios.get(apis.base + apis. ThisWeekPendingOrderPercentage, {
              headers: {
                userId: 30,
              },
            }).then((succResp) => {
                callbackPendingOrders(succResp);
              }).catch((errorresp) => {
                console.log(JSON.stringify(errorresp));
              });
          };

        const fetchThisWeekAppUsersData = () => {
            Axios.get(apis.base + apis.ThisWeekAppUsersPercentage, {
              headers: {
                userId: 30,
              },
            }).then((succResp) => {
                callbackThisWeekAppUsers(succResp);
              }).catch((errorresp) => {
                console.log(JSON.stringify(errorresp));
              });
          }; 
        
          const fetchNewOrdersData = () => {
            Axios.get(apis.base + apis.ThisWeekNewOrderPercentage, {
              headers: {
                userId: 30,
              },
            }).then((succResp) => {
                callbackNewOrders(succResp);
              }).catch((errorresp) => {
                console.log(JSON.stringify(errorresp));
              });
          }; 
          ...
          const fetchBestSellerData = () => {
            Axios.get(apis.base + apis.adminBestSeller, {
              headers: {
                userId: 30,
              },
            }).then((succResp) => {
                callbackBestSeller(succResp);
              }).catch((errorresp) => {
                console.log(JSON.stringify(errorresp));
              });
          };
        ...
        const callback = (res) => {
            setData({...data,on_going:res.data});
        };

        
        const callbackCancelledOrders = (res) => {
            setData(prevData => ({
                ...prevData,
                this_week_can: res.data.this_week_can, 
                diff_can: res.data.diff_can, 
            }));
        };

        const callbackPendingOrders = (res) => {
            setData((prevData) => ({
              ...prevData,
              this_week_pen: res.data.this_week_pen,
              diff_pen: res.data.diff_pen,
            }));
          };

        const callbackNewOrders = (res) => {
            setData((prevData) => ({
              ...prevData,
              new_orders: res.data.this_week_ord,
              diff_new_orders: res.data.diff_ord,
            }));
          };

        const callbackThisWeekAppUsers = (res) => {
            setData((prevData) => ({
              ...prevData,
              this_week_user: res.data.this_week_user,
              diff_user: res.data.diff_user,
            }));
          };

        const callbackBestSeller = (res) => {
            setData((prevData) => ({
              ...prevData,
              best_seller: res.data,
            }));
          };
